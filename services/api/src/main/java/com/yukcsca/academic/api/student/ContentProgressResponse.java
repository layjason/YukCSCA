...
public record ContentProgressResponse(String status, Integer resumeBlockIndex, Instant updatedAt) {
  static ContentProgressResponse from(ContentProgressProjection value) {
    return new ContentProgressResponse(value.status(), value.resumeBlockIndex(), value.updatedAt());
  }
}
