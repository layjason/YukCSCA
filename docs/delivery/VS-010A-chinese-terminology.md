...
| Related ADRs                 | [`ADR-0002`](../decisions/ADR-0002-edge-tts-term-pronunciation.md) — Edge TTS for pre-rendered term audio (supersedes `ADR-0001`)                                                                                                                                                                                  |
...
| API operations               | Admin draft: `terms[]`, `requiredTermIds`, `authoredTermAttachments`. **1** admin audio GET (`CR-02`). **9** student ops: 8 academic terminology + 1 assessment Language help. Additive optional fields on lesson browse/read and `AssistanceSummary`. Required `SessionItemView.languageHelpAvailable` (`CR-01`). |
...
| CR-applied checkpoint        | `VS-010A-R17-cr-applied` — `CR-04` inline `\(...\)` in TEXT-like prose; hashes in verification evidence. Prior: `VS-010A-R10-cr-applied` (`CR-01`).                                                                                                                                                                |
| Accepted contract checkpoint | `VS-010A-R17-cr-applied` — product-owner requested implementation in the same change. Prior accepted wire `VS-010A-R10-accepted` remains the student-ops baseline.                                                                                                                                                 |
...
- Author a package-scoped **term bank**. Each term has one identity, Chinese surface form(s), pinyin, explanation-language definitions (`id` / `en` / `zh-CN`), English equivalent, **term class**, topic bindings, and optional symbols and example. Meaning for students is the explanation-language gloss — there is no separate domain-meaning field (`D-09`). Admin pinyin is auto-filled from the Chinese term and stays editable for polyphone correction. Optional symbols are KaTeX-compatible LaTeX. Lesson, question, and remediation **TEXT** blocks, plus term definitions, English equivalent, and example, may mix prose with bounded inline KaTeX (`\(...\)`). Display-mode formulas stay MATH blocks. Drafts show no live audio; Play appears only against the stored published clip.
- Bind a **small required set on each LESSON**. Exam-wording terms (`EXAM_INSTRUCTION`, `LOGICAL_EXPRESSION`) are authored once and reused across topics. `TOPIC_TERM` entries still carry outline ids for authoring, but the student preview and lesson rail use **this lesson’s** `requiredTermIds`. Several lessons may share one outline node; each lesson only surfaces its list.
- Student preview is that lesson’s required list (the LESSON id is the preview resource). A `TERMINOLOGY` study resource is not a shared outline dictionary dumped into every lesson.
...
- Opening a LESSON that has required terms offers **this lesson’s** preview first (or resumes an unfinished one).
- Cards show characters, **pinyin always**, optional Play, definition, and English equivalent. Each card has a **bookmark** control on the right. **Bookmark all** adds this lesson’s list only.
- **Practice these terms** (matching pairs) is optional. **Continue to lesson** is never locked.
- Opening or completing preview does **not** write the notebook. Bookmark = add; unbookmark = remove (even if it was saved earlier). Unbookmark means the student already knows this and does not want it in the notebook.
...
- After Language help, underlined spans stay. There is **no Key phrases tray**. Hover (wide) or tap (phone) opens a **compact gloss bubble** (explanation-language meaning centered beside bookmark/unbookmark — it does not restate the Chinese surface), not the full term card. Bookmarking shows a toast. Lookup of unknown selected text stays `NOT_IN_BANK` and is not a student-authored term.
...
- At publish, pre-render a short clip per Chinese surface form through `SpeechSynthesisPort` using Edge TTS (`edge_tts` protocol) with `zh-CN-XiaoxiaoNeural`. Store the `audio/mpeg` bytes like VS-005 images (PostgreSQL, not object storage).
- Draft authoring has no live preview; the admin sees “Audio is generated upon publishing.”
- After publish, the admin Play control and the student Play control both return the stored clip for that surface form.
- Missing or failed audio hides Play and leaves pinyin visible.
- Student and admin audio GETs never synthesize.
...
- Student-created definitions or extra terms that are not in the reviewed bank. Bookmarking a reviewed term is in scope; inventing a word is not.
...
| Scored item   | Clean Chinese stem + quiet Language help               | Underlined spans; compact gloss bubble (not the full card)   |
...
Clean Chinese stem
  ...
  Same stem, with word/phrase chips (admin-preset ∪ auto-match)
        │
        ▼  hover (wide) or tap (phone) an underlined span
  Compact gloss bubble (explanation-language meaning + bookmark; no restated Chinese)
        ├─ not bookmarked → empty bookmark → toast on add
        └─ already        → filled bookmark → unbookmark
  Full card (pinyin / Play / example) stays on notebook and explicit lesson open.
```
...
- Fallbacks: no terms → no terminology chrome. Missing explanation-language gloss is explicit, never a silent substitute. Missing audio → Play unavailable, pinyin visible. Speech down at publish → package may still publish. Non-`STUDENT` → 403 on student APIs. Tests never call Microsoft Edge TTS.
...
4. Required terms are **not** auto-added. The student bookmarks what they want. The LESSON shows that lesson’s rail.
5. On a checkpoint or practice item the stem is clean. If wording blocks them, they open Language help, then hover or tap an underline for a compact meaning bubble. Bookmark from the bubble if they want the term in the notebook.
...
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------- | ----------------------------------- |
| AC-01 | Given a published topic with a required set, when the student opens the preview, then each required term shows characters, pinyin, explanation-language definition, and English equivalent under one term id. Pinyin stays visible if Play is present, missing, or failed.                                                              | Preview cards; language-unavailable explicit; Play optional | One term identity; no silent language substitute                 | Preview exposure; not mastery      | Contract + academic IT + preview UI |
| AC-02 | Given preview completion, then viewing does not write `CHECKPOINT_PASSED` or any mastery signal, and required terms are **not** auto-added to the notebook. Matching pairs are optional and do not lock Continue to lesson. Bookmark / unbookmark / Bookmark all apply to this lesson’s list only.                                      | Continue to lesson; bookmark on each card                   | Preview PUT does not upsert notebook                             | Bookmark uses `CLICKED`            | Domain + UI                         |
| AC-03 | Given the LESSON, then that lesson’s rail is available and required/instruction forms may be tappable. Given a scored item, the stem stays clean until Language help is opened; then underlines stay and a compact meaning-only gloss bubble opens on hover/tap, not the full card.                                                     | Lesson rail; quiet Language help; no Key phrases tray       | Spans computed at publish; item presentation is on request       | Lookup does not write notebook     | Reader + player                     |
| AC-04 | Given a matching selection after Language help is open, then the student sees a compact explanation-language gloss, and the original item stays in Chinese.                                                                                                                                                                             | Gloss bubble over unchanged stem                            | No full-question translation                                     | `LANGUAGE_ASSIST`; bookmark opt-in | Player                              |
...
| AC-06 | Given an explicit bookmark from preview, lesson, gloss bubble, or notebook, then one notebook row exists. Unbookmark removes it. Lookup of unknown text is `NOT_IN_BANK` and writes nothing.                                                                                                                                            | One list; bookmark / unbookmark                             | Idempotent upsert; delete is idempotent                          | Notebook + event                   | Domain + UI                         |
...
| AC-09 | Given a missing explanation-language gloss, then the limitation is explicit and no other language is substituted.                                                                                                                                                                                                                       | Visible fallback                                            | No implicit language swap                                        | Unchanged term identity            | API + UI                            |
...
| AC-11 | Given a stored clip, when the student or admin activates Play, then authorized audio plays. Given a missing clip, Play is unavailable and pinyin stays visible.                                                                                                                                                                         | Play; missing-audio state                                   | Speech only at publish; GET never synthesizes                    | Bounded audio bytes                | Port mock + GET + UI                |
...
| Technology/dependency need, alternatives, and ADR threshold   | PLAN dependency table; `ADR-0002`              | `CLEAR` | Edge TTS at publish only. Tests mock the port.                                                        |
...
| Approval scope    | `D-01`–`D-11` as recorded. Student wire baseline remains `VS-010A-R10-accepted`; `CR-05` (`D-11`) is `VS-010A-R18-cr-applied`. |
...
| `D-09` | Keep a separate domain-meaning field besides three-language explanations? | **No.** Remove `domainMeaning`. Student meaning is the explanation-language gloss plus English equivalent. Extra meaning field caused authoring confusion.                                                                                                                                       | Product owner | `APPROVED` | 2026-08-19. `CR-03`.                                                                                |
| `D-10` | Replace Azure Speech with Edge TTS (`edge_tts`)?                          | Yes. Keep publish-time `SpeechSynthesisPort`, voice `zh-CN-XiaoxiaoNeural`, and pinyin fallback. Java speaks the Edge read-aloud protocol. No Python sidecar. Admin Play uses stored published bytes.                                                                                            | Product owner | `APPROVED` | 2026-08-19. `ADR-0002` supersedes `ADR-0001`.                                                       |
| `D-11` | Vocab opt-in; lesson-bound preview; compact practice gloss?               | **Yes.** Bind required terms on the lesson, not a shared outline preview. Notebook is bookmark-only (no auto-add on preview open/complete). Practice/checkpoint after Language help: underlines stay, no Key phrases tray, compact gloss bubble with bookmark. Unknown lookup stays not in bank. | Product owner | `APPROVED` | Journey review 2026-08-20. `CR-05`.                                                                 |
...
- One `termId` per authored term inside a package. Explanation-language glosses do not fork identity. The same operations serve any subject; they are not Mathematics-only routes.
...
| ----------------------- | --------------------------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Get topic preview       | `GET /api/v1/academic/packages/{subject}/terminology/{resourceId}`          | `STUDENT` | This LESSON’s required cards + optional match targets. `resourceId` is the LESSON id.                                 | 401 / 403 / 404 / 400                                                           |
| Upsert preview progress | `PUT .../terminology/{resourceId}/progress`                                 | `STUDENT` | Progress only; does **not** write the notebook                                                                        | 401 / 403 / 404; `FORMAL_ASSISTANCE_DISABLED`; `TERMINOLOGY_VALIDATION_FAILED`  |
| Bookmark lesson terms   | `POST .../terminology/{resourceId}/bookmarks`                               | `STUDENT` | Bookmarks this lesson’s required list (or a subset)                                                                   | 401 / 403 / 404; `FORMAL_ASSISTANCE_DISABLED`; `TERMINOLOGY_VALIDATION_FAILED`  |
...
| Resolve lookup          | `POST /api/v1/academic/term-lookups`                                        | `STUDENT` | `MATCHED` card without writing the notebook, or `NOT_IN_BANK`                                                         | 401 / 403 / 404; `FORMAL_ASSISTANCE_DISABLED`; validation                       |
| Bookmark term           | `PUT /api/v1/academic/terminology-notebook/{termId}`                        | `STUDENT` | Adds one reviewed term. Idempotent.                                                                                   | 401 / 403 / 404; `FORMAL_ASSISTANCE_DISABLED`; validation                       |
| Unbookmark term         | `DELETE /api/v1/academic/terminology-notebook/{termId}`                     | `STUDENT` | Removes the row. Idempotent when already absent.                                                                      | 401 / 403; `FORMAL_ASSISTANCE_DISABLED`                                         |
...
Admin: existing draft routes plus **`CR-02`** `GET /api/v1/admin/academic-packages/{id}/terms/{termId}/audio` (stored published clip only). `AcademicPackageDraft.terms[]`; `StudyResource.requiredTermIds` on **LESSON** (student preview list); `Question.authoredTermAttachments`. Publish validation (implementation) requires Chinese surface and pinyin; `TOPIC_TERM` should have at least one outline id.
...
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------- |
| `TermDraft`          | `id`, `termClass`, `surfaceForms[]` (text + pinyin), `definitions`, `englishEquivalent`, optional symbols/example, `outlineItemIds`       | Class required. ≥1 surface form; pinyin required; missing explanation-language gloss allowed and flagged. Topic class should have ≥1 outline id at publish. TEXT-like strings may include `\(...\)`; unmatched or unsafe fragments fail publish. | Academic package revision |
| `TerminologyPreview` | LESSON resource id + that lesson’s `requiredTermIds[]` + student cards + `lessonResourceIds` (this lesson) + optional match targets       | Required ids must exist in the same draft                                                                                                                                                                                                        | Academic                  |
| `TermCard`           | `termId`, `subject`, `termClass`, surfaces, `definition` union (`AVAILABLE` \| `LANGUAGE_UNAVAILABLE`), `englishEquivalent`               | No silent language substitute                                                                                                                                                                                                                    | Academic                  |
...
- Collection: GET preview is side-effect free. `PUT` progress never writes the notebook. Bookmark is opt-in (`PUT` one term, `POST` this lesson’s list, `DELETE` to remove). Lookup does not upsert.
...
- Audio: publish-time `audio/mpeg` only. Student and admin GETs never synthesize. Optional `surfaceForm` selects a non-primary clip.
...
| `CR-02` | Admin must hear the **exact stored clip students hear** after publish. Draft authoring must not live-synthesize. The student audio GET is `STUDENT`-only.                                                                                                                                                                                                                                                                                                                                                                               | Add `GET /api/v1/admin/academic-packages/{id}/terms/{termId}/audio` (`ADMIN`). Serves the active published revision. Optional `surfaceForm`. Never synthesizes. 404 when unpublished or missing.                                                                                                                                                                                                             | **ACCEPT.** Product-owner request 2026-08-19. Additive. Same bytes as the student GET. `D-10`.                                                                                                                                                                                                    | `D-10`            | `RESOLVED` | Applied in `academic-admin.tsp`; regenerated OpenAPI/web.                                                                                                                                      |
| `CR-03` | Separate `domainMeaning` confused authors who already write three-language explanations.                                                                                                                                                                                                                                                                                                                                                                                                                                                | Remove `domainMeaning` from `TermDraft` and `TermCard`. Publish no longer requires it.                                                                                                                                                                                                                                                                                                                       | **ACCEPT.** Product-owner `D-09`. Breaking attribute removal on an unpublished slice.                                                                                                                                                                                                             | `D-09`            | `RESOLVED` | Applied in TypeSpec + implementation.                                                                                                                                                          |
| `CR-04` | Authors must split a sentence into TEXT / MATH / TEXT or leave symbols as raw text. MATH `displayMode` is a whole block, so students see `f'(x)` instead of rendered math inside prose. The same gap appears on lesson/question/remediation TEXT, Language help stems, and term definition/example/English equivalent.                                                                                                                                                                                                                  | Keep the TEXT string. Allow bounded inline KaTeX between `\(...\)`. MATH stays for standalone/display formulas. No new JSON field or Flyway table: still `academic_package.draft` / `academic_revision.content` JSONB. Publish validates fragments with the existing latex safety rules. Terminology spans skip inline-math ranges; offsets stay UTF-16 into the source string.                              | **ACCEPT.** Product-owner request 2026-08-19. Additive documentation and validation on existing strings. Same KaTeX bound as MATH. No scored-state, privacy, or new operation.                                                                                                                    | —                 | `RESOLVED` | Applied in TypeSpec docs + publish validation + shared `MixedProse` renderer. Checkpoint `VS-010A-R17-cr-applied`.                                                                             |
| `CR-05` | Journey review: outline-shared `TERMINOLOGY` preview dumps every lesson’s vocab; preview PUT auto-collects into the notebook; Language help opens a full term card plus a Key phrases tray.                                                                                                                                                                                                                                                                                                                                             | Bind `requiredTermIds` on LESSON. Preview `resourceId` is the lesson. PUT progress does not collect. Lookup does not write. Add bookmark PUT/POST and unbookmark DELETE. Compact gloss on scored items.                                                                                                                                                                                                      | **ACCEPT.** Product-owner `D-11` journey review 2026-08-20. Product-meaning change on an unpublished slice; no Flyway table change.                                                                                                                                                               | `D-11`            | `RESOLVED` | Applied in TypeSpec + backend + Learn/Practice UI. Checkpoint `VS-010A-R18-cr-applied`.                                                                                                        |
...
| Cards: characters, pinyin, explanation definition, English equivalent; one `termId` | `TermCard` + `TermDefinition` union                                                                    | **Sufficient** (AC-01, AC-09)                                            |
...
| Admin term bank | Empty bank; editor validation (surface, pinyin, class, topic bindings); publish violations mapped to fields; do not require the admin to mark every question character                                                                                                                                             |
...
- **Student goal:** Chinese exam wording must not hide the subject. Jia opens a topic, sees a short required list (characters, always-visible pinyin, optional Play, her explanation-language definition, English equivalent), may skip the pairs game, then studies the lesson with a rail if she forgets. On a checkpoint the stem stays a real exam stem. She asks for help. The original Chinese does not disappear. The notebook is one list she can trust — Due first, not four homes.
...
- Publish rejects a required term missing a Chinese surface form or pinyin.
...
8. ~~Integrate the real HTTP flow. Contract, backend, and frontend automated evidence are recorded. Product-owner journeys remain.~~ **Done** — automated evidence recorded above; product-owner journey review passed 2026-08-22 with zero open defects.
9. ~~Slice owner advances lifecycle only after both sides record evidence **and** the actor completes the real flow.~~ **Done** — actor completed the real flow; status `DONE` (revision 21).
10. ~~Verify privacy, failure recovery, mobile/desktop, reduced motion, and localization before `DONE`.~~ **Done** — verified in the 2026-08-22 review.
...
- [x] Delivered flow matches in-scope and out-of-scope lists.
...
- [x] Authorization, privacy, minor safety, and audit were reviewed.
- [x] Mobile, accessibility, localization, low-bandwidth, reduced-motion, and failure states were verified where they apply.
- [x] Routes, components, and prototype reuse or deletion are recorded.
- [x] UI follows `DESIGN.md`. Any shared token change was made there first.
- [x] Journey handoffs are coherent. Prototype-only behavior stays isolated.
...
| Technology/ADR review        | Edge TTS only at publish behind `SpeechSynthesisPort`; tests use `TestSpeechSynthesisPort`. CI does not contact Microsoft Edge TTS.                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Frontend visual review       | **Passed 2026-08-22** — 390px and 1280px, keyboard focus order, reduced motion, and `id` / `en` / `zh-CN` chrome verified during the product-owner journey review. No `DESIGN.md` token change.                                                                                                                                                                                                                                                                                                                         |
| End-to-end/manual flow       | **Product-owner journey review 2026-08-22 — all flows passed** against the local Mathematics seed: admin authoring, per-lesson preview (bookmark / unbookmark / Bookmark all, no auto-collect), lesson rail and tappable spans, clean scored-item first paint with quiet Language help, compact gloss with bookmark toast, `NOT_IN_BANK` lookup, wording-hard cause, notebook due cloze/pairs, missing-`id` gloss `LANGUAGE_UNAVAILABLE`, audio Play/404, and English/no-term-package chrome omission. No open defects. |
...
Named coverage: `AcademicDraftProcessorTest` (omit terms; missing surface/pinyin; TOPIC_TERM outline; unknown requiredTermIds/attachments), `TerminologyProjectorTest` (UTF-16/alias; no extra TOPIC_TERM auto-match; admin-preset can add a non-required `TOPIC_TERM`), `StudentTerminologyNotebookTest` (idempotent upsert + familiarity/due), `FormalAssistancePolicyTest`, `CheckpointPassEvaluatorTest` (language assist is not a pass input), `AcademicTerminologyHttpIT` (preview GET/PUT/check, lookup MATCHED/NOT_IN_BANK, notebook/review, audio 200/404, English omit chrome, formal 403), `AssessmentLanguageHelpHttpIT` (English `languageHelpAvailable=false` + disclose 409; zh-CN disclose spans only required + exam-wording; `languageAssistUsed=false`; checkpoint pass; revalidation pass with only LANGUAGE_ASSIST; formal 403). Existing `AcademicStudentHttpIT` / `AssessmentStudentHttpIT` / `AcademicAdminHttpIT` still pass without `terms[]`.
...
- Lesson explanation-language toggle (ID / EN / 中文): the Checkpoint strip stays in document flow under the lesson. **Diagnosis:** if the student has not scrolled to it, it must remain below the fold. Unmounting or shrinking the body pulls that footer into the viewport. Pinning a second copy in the viewport (`position: fixed`, scroll-anchor, or a scroll-delta re-pin) also makes it appear while the original is still off-screen — that was the flash. **Fix:** keep the previous body and content-progress mounted; grow-only `min-height` on the column above the footer; restore `.app-content-wrapper` scroll (the shell scroller, not `window`); do not pin the footer. Page-enter and Checkpoint-enter animations do not replay. `CheckpointCta` keeps a per-lesson ready cache. Local `make dev` serves Docker nginx — rebuild `web` before `:5173` shows this. Product-owner journeys remain.
...
### Local Mathematics journey seed (2026-08-21)

The published local Mathematics package was updated so product-owner review can
exercise the lesson-bound preview and compact gloss without authoring from
scratch. This is **local development data**, not a Flyway seed and not a
production content claim.

| Surface                                 | What to open                                               | What it covers                                                                                                                                                                  |
| --------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lesson 1 preview                        | `Basic sets and inequalities`                              | This lesson’s required list (field-strength topic terms + 集合), not the outline dictionary. Bookmark / unbookmark / Bookmark all. Opening preview does not write the notebook. |
| Lesson 2 preview                        | `Domain and derivative wording` (same outline as lesson 1) | Sibling lesson with a **different** required list (`定义域`, `导数`, `公因式`). Proves preview is per-lesson. `公因式` has no Indonesian gloss.                                 |
| Lesson 3                                | `Interval notation without a preview`                      | Empty `requiredTermIds` — no terminology intercept before the lesson. Rail empty.                                                                                               |
| Lesson 1 body                           | Chinese version                                            | Tappable required/instruction spans; full card (not the compact gloss).                                                                                                         |
| Chinese checkpoint / immediate practice | Existing field-strength items                              | Language help → compact meaning gloss, filled bookmark, toast, no Key phrases tray. Authored attachments + auto-match (`如图`, `则`, `求`).                                     |
| Chinese checkpoint on lesson 2          | `Checkpoint: find the domain`                              | Authored `定义域` + `求` (attachments are exclusive when they overlap the auto set).                                                                                            |
| Immediate practice                      | `If and only if wording`                                   | `LOGICAL_EXPRESSION` (`当且仅当`).                                                                                                                                              |
| SET_END practice                        | `Wording after submit`                                     | Look-up other text; `韦达定理` is `NOT_IN_BANK`. Wording-hard after submit.                                                                                                     |
| Notebook                                | `/app/learn/terms`                                         | Existing bookmarked rows plus new bookmarks; unbookmark toast; due cloze/pairs when snippets exist.                                                                             |
| Missing Indonesian gloss                | `公因式`                                                   | `LANGUAGE_UNAVAILABLE` when explanation language is `id`.                                                                                                                       |
...
| -------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 21       | 2026-08-22 | Product-owner journey review passed on the local Mathematics seed: all flows (admin authoring through notebook due review) completed with no open defects; visual/accessibility review (390/1280, keyboard, reduced motion, `id`/`en`/`zh-CN`) passed. Status `CONTRACT_READY` → **`DONE`**. PLAN `0.5.43`.                                  |
| 20       | 2026-08-21 | Lesson language-switch Checkpoint flash: recorded the actual diagnosis (below-fold strip pulled or pinned into the viewport) and the working rule (in-flow footer, grow-only column height, shell scroll restore). Removed retry notes. Status remains `CONTRACT_READY`.                                                                     |
| 19       | 2026-08-21 | Compact gloss is meaning-only (no restated Chinese surface); filled bookmark vs empty outline; toast on bookmark. Local Mathematics package bound per lesson for product-owner journey review. Status remains `CONTRACT_READY`.                                                                                                              |
| 18       | 2026-08-20 | `D-11` / `CR-05`: lesson-bound preview; notebook is opt-in bookmark (no auto-add); practice/checkpoint compact gloss bubble, no Key phrases tray. PLAN `0.5.42`. Status remains `CONTRACT_READY`.                                                                                                                                            |
| 17       | 2026-08-19 | `CR-04`: TEXT-like prose may include bounded `\(...\)` inline KaTeX (lessons, questions, remediation, term definition/example/English equivalent). MATH stays display/standalone. No new table; JSONB source string unchanged. PLAN `0.5.41`. Status remains `CONTRACT_READY`.                                                               |
| 16       | 2026-08-19 | `D-09` / `CR-03`: remove `domainMeaning` from term identity, cards, and publish validation. Meaning is the explanation-language gloss. PLAN `0.5.40`. Status remains `CONTRACT_READY`.                                                                                                                                                       |
| 15       | 2026-08-19 | Product-owner authoring follow-ups: auto pinyin, alias delete, KaTeX symbols, plain labels, Edge TTS (`D-10` / `ADR-0002`), admin published Play (`CR-02`). PLAN `0.5.39`. Status remains `CONTRACT_READY`. Not `DONE`.                                                                                                                      |
