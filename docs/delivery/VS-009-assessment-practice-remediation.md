...
| Accepted contract checkpoint | `VS-009-R8-accepted` — current wire after `CR-05` (`checkpointUpdatedSinceLastAttempt` on `CheckpointForLesson`). `VS-009-R5-accepted` remains the historical pre-CR-05 checkpoint. |
...
**Initialized** at revision 3 as checkpoint `VS-009-R3-initial`. **CR-01–CR-04 applied** at revision 5 as `VS-009-R5-cr-applied`. **Accepted** at revision 6 as `VS-009-R5-accepted` (identical wire hashes after FE re-review). **`CR-05` recorded** at revision 8 as `VS-009-R8-accepted` (required `checkpointUpdatedSinceLastAttempt` on `CheckpointForLesson`). Executable sources are TypeSpec; this table is the slice index.
...
| `getCheckpointForLesson`  | `GET /api/v1/assessment/packages/{subject}/lessons/{resourceId}/checkpoint` | STUDENT | startable, lockReason, `checkpointUpdatedSinceLastAttempt`, editions[] | 401/403/404                                           |
...
| `CheckpointForLesson`            | startable, lockReason, **`checkpointUpdatedSinceLastAttempt`** (CR-05; soft signal only), editions by exam language                                        |
...
| Frontend consumer re-review                                | Frontend owner      | `COMPLETE` | Revision 6 re-review of `VS-009-R5-cr-applied`; accepted as `VS-009-R5-accepted`; later `CR-05` recorded in revision 8 |
| Accepted contract checkpoint recorded                      | Backend + frontend  | `COMPLETE` | Current accepted checkpoint **`VS-009-R8-accepted`** (`CR-05`); `VS-009-R5-accepted` is historical                     |
...
| `CR-05` | Learn checkpoint CTA and checkpoint page must tell the student when published CHECKPOINT questions changed after a prior submitted attempt, without demoting progress or claiming mastery. R5 `CheckpointForLesson` lacked this field; FE later hard-required it. | Required `checkpointUpdatedSinceLastAttempt: boolean` on `CheckpointForLesson`. Soft signal only.          | **ACCEPT.** Additive required field; already implemented and consumed. Records the post-R5 wire so `CONTRACT_READY` hashes match the current TypeSpec. No new scored-state meaning.                                            | —                 | `RESOLVED` | Applied on current wire; recorded `VS-009-R8-accepted` (rev 8)                                                     |
...
6. ~~Frontend re-review~~ **Done (rev 6)** — accepted `VS-009-R5-accepted`; **`CR-05` recorded later (rev 8)** as `VS-009-R8-accepted`; status **`CONTRACT_READY`**.
...
9. ~~Frontend: Learn handoff; Practice; player; mistakes; remediation reader (from `VS-009-R5-accepted` only).~~ **Done (rev 7)** — production `features/assessment` + Learn checkpoint CTA; Practice/Mistakes promoted off preview.
10. Integrate real package early; verify AC-20 agent context fields.
11. ~~Update `ARCHITECTURE.md`, `PLAN.md`, `COVERAGE.md`; `DONE` only after product-owner accepts the three journeys.~~ **Done (rev 9)** — product-owner journey review 2026-08-14; status `DONE`.

## Definition of done

- [x] Story mapping honest (full US-COURSE-03 / US-PRACTICE-01 / US-MISTAKE-01–03; partial US-HINT-01).
- [x] Documentation sufficiency complete; `D-01`–`D-16` reflected in contract/code.
- [x] Human gate remains `APPROVED` (or reopened with evidence).
- [x] TypeSpec compiles; initial + accepted checkpoints; FE consumer review; no open CR (`VS-009-R8-accepted`).
...
- [x] All AC rows have named evidence (backend IT/unit + frontend component/API; AC-10/11/13/15/19 and end-to-end journeys A–C via product-owner review 2026-08-14).
- [x] Authorization, privacy (private notes), no key leakage reviewed.
- [x] Mobile, a11y, localization, reduced motion verified for Practice/checkpoint/mistakes (product-owner journey review).
- [x] Architecture/PLAN/COVERAGE updated; agentic ports documented without claiming VS-011 done.
- [x] Product owner accepts the three real journeys above.
...
| CR disposition                | All `CR-01`–`CR-05` **RESOLVED**; **zero open CR**                                                                                                                                                                                                                                           |
| Accepted contract checkpoint  | **`VS-009-R8-accepted`** — current wire including `CR-05`. `VS-009-R5-accepted` remains historical. Slice status **`DONE`** after product-owner journey review 2026-08-14.                                                                                                                   |
| Technology/ADR review         | Complete at shaping — existing stack + new assessment module                                                                                                                                                                                                                                 |
| Backend tests                 | 2026-08-11: unit + Testcontainers HTTP IT for assessment + academic extensions — **PASS** (`./mvnw -Dtest=com.yukcsca.assessment.**,com.yukcsca.academic.**`; Flyway V9 applied; BUILD SUCCESS). Later uniqueness indexes V10–V11 remain in the same assessment store.                       |
| Frontend tests / visual / e2e | `pnpm typecheck:web`, `pnpm lint:web`, focused assessment/admin/shell vitest — green through residual honesty/resume/hub work; product-owner visual review 2026-08-14                                                                                                                        |
| Frontend production surfaces  | Practice hub, session player, result, mistakes, remediation, Learn checkpoint CTA under `features/assessment`; Practice/Mistakes/checkpoint/remediation promoted off preview                                                                                                                 |
| End-to-end / product owner    | **`ACCEPTED — PRODUCT_OWNER_REVIEW` (2026-08-14):** journeys A (checkpoint pass / fail → remediation → revalidation), B (topic practice → mistake loop), C (resume). Honesty repair accepted: hinted Recheck is assisted, not fail. Adjacent shell/admin polish is out of VS-009 acceptance. |
| Completion disposition        | Contract (`VS-009-R8-accepted`), backend/database (V9+), production frontend, security/privacy, prototype isolation, and product-owner journey evidence are complete; VS-009 moved to `DONE` without selecting the next production slice.                                                    |
...
**`VS-009-R5-accepted`:** identical hashes to the table above (FE re-review rev 6; no wire change). Historical — superseded by `VS-009-R8-accepted` after `CR-05`.

### `VS-009-R8-accepted` artifact hashes (`git hash-object`)

| Artifact                                       | Hash                                                                                        |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `contracts/assessment-student.tsp`             | `65626e901a884948c8345d9a5f19590806fe33e8`                                                  |
| `contracts/academic-admin.tsp`                 | `e7b6d7785b051476b989040f6487dc521c51a292` (unchanged vs R5)                                |
| `contracts/academic-student.tsp`               | `2bf32a6a0c71863955cf53dc967334efabcd76cc` (doc-only `updatedSinceCompleted` clarification) |
| `contracts/main.tsp`                           | `ea6d742258f200e86d29cd62b4868271498ae9cf` (unchanged vs R5)                                |
| `contracts/generated/openapi.yaml`             | `a69d82c6684a861989f68fa75d2109d479ee3f1c`                                                  |
| `apps/web/src/shared/api/generated/openapi.ts` | `deb6d6c671779f063bb54523fc9a715d156508d7`                                                  |
...
| -------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 9        | 2026-08-14 | Product-owner journey review accepted for checkpoint pass, fail→remediation→revalidation, topic practice→mistake, and resume. Closed remaining definition-of-done and journey evidence; synchronized PLAN/ARCHITECTURE/COVERAGE; moved VS-009 from `CONTRACT_READY` / `IN_PROGRESS` to `DONE` without selecting the next production slice.                                                                                                                                                                                                                                                            |
| 8        | 2026-08-13 | Recorded post-R5 additive required field `CheckpointForLesson.checkpointUpdatedSinceLastAttempt` as `CR-05` / accepted checkpoint **`VS-009-R8-accepted`**. Status remains `CONTRACT_READY`; product-owner journeys still required.                                                                                                                                                                                                                                                                                                                                                                   |
| 7        | 2026-08-11 | Student frontend implementation from `VS-009-R5-accepted`: `features/assessment` API adapter (DEV fallback policy aligned with Learn), Practice hub + Continue, session player (soft STRONG confirm, REVALIDATION STRONG off, IMMEDIATE/SET_END), result without mastery claims, mistakes list/detail, remediation reader + revalidation handoff, Learn checkpoint CTA; Practice/Mistakes/checkpoint/remediation routes promoted to `student-settings`/`implemented`; en/id/zh-CN; focused tests green. Status remains `CONTRACT_READY` pending backend assessment module and product-owner journeys. |
