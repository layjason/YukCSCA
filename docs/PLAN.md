...
| Current production slice     | [`VS-009`](delivery/VS-009-assessment-practice-remediation.md) assessment practice and remediation — backend implemented; FE remaining (`CONTRACT_READY`) |
...
| [`VS-008`](delivery/VS-008-student-learn-content.md)           | Browse a published package’s official-source outline and product coverage, open one LESSON (TEXT/MATH/IMAGE) with id/en/zh-CN explanation-language toggle, and resume content progress (not mastery)                                                                                             | Partial `US-COURSE-01`, partial student `US-SYL-01`, partial `US-COURSE-02`, `US-LANG-02` | `VS-004`, `VS-005`                                       | `DONE`           |
| [`VS-009`](delivery/VS-009-assessment-practice-remediation.md) | Complete a lesson checkpoint or topic-practice set with tiered math hints and close one mistake → remediation → revalidation loop on a shared assessment substrate                                                                                                                               | `US-COURSE-03`, `US-PRACTICE-01`, partial `US-HINT-01`, `US-MISTAKE-01`–`03`              | `VS-005`, `VS-008`                                       | `CONTRACT_READY` |
...
- `VS-009` proves plan-independent checkpoint **and** topic practice on one assessment substrate (attempts, math hint tiers, mistakes, published REMEDIATION, immediate revalidation, bounded objective evidence), shaped for later agent tools without shipping the agent. Chinese language-assist and terminology remain `VS-010`. `VS-017` later adds assigned practice through an active plan.
...
| 0.5.24 | 2026-08-11 | Deep-research refine of [`VS-009`](delivery/VS-009-assessment-practice-remediation.md) to revision 2: Bloom formative→corrective→retest and VanLehn inner/outer-loop mapping; feedback modes; session resume; remediation resolution; revalidation alternate-item policy `D-16`; agent context envelope; expanded ACs. Status remains `SHAPING`. |
| 0.5.23 | 2026-08-11 | Selected and shaped `VS-009` as the student assessment substrate: lesson CHECKPOINT + plan-independent TOPIC_PRACTICE, tiered math hints, mistake → published REMEDIATION → immediate revalidation, bounded objective evidence, new `assessment` module with agentic-ready evidence ports (no LLM in this slice). Created [`VS-009-assessment-practice-remediation.md`](delivery/VS-009-assessment-practice-remediation.md) revision 1 (`SHAPING`); grill decisions `D-01`–`D-15` approved. Chinese language-assist remains `VS-010`. |
| 0.5.22 | 2026-08-10 | Recorded product-owner acceptance of the VS-008 production student Learn journey (browse published package, LESSON reader, explanation-language toggle, content progress) after findings fixes; moved VS-008 to `DONE`; set current baseline to VS-008 and left the next production slice unselected. Checkpoint/practice (`VS-009`), multi-subject seeding, entitlements, and parent syllabus remain later. |
