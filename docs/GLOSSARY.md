...
- **Terminology entry:** One canonical reviewed term identity for a **Chinese exam-language** CSCA package: surface form(s), pinyin, explanation-language definition, English equivalent, and domain meaning. First published use is Chinese Mathematics; the same kind of entry is reused for later Chinese Physics and Chemistry. A `TERMINOLOGY` study resource is the topic preview unit that lists required entries; it is not itself the term identity. **Avoid:** treating a lesson page, an unreviewed dictionary hit, or “Mathematics-only vocabulary” as the product concept.
...
- **Terminology notebook:** The student’s collected terminology entries from required course terms, in-context lookups, and language-related mistakes, with familiarity and due-review state. Viewing or collecting a term is not topic mastery.
- **Language assistance:** Help for exam-language wording (word or phrase meaning, and later authored stronger tiers). It is recorded separately from a mathematical hint. Strong language assistance is not independent mastery.
