...
| 4.3 | Focused Learning-Unit Content | `US-COURSE-02`, `US-COURSE-04`, `US-ADM-08` | Owners: delivered `VS-005` (admin LESSON/TEXT/MATH/IMAGE); delivered `VS-008` (student LESSON reader + content progress); proposed `VS-010B` (reviewed short video upload or template-bound scene specification → review → student play). V1.8 question-explanation attachment is owned by an unnamed additive follow-on after `VS-010B`. |
...
| 4.5 | Chinese Mathematics Terminology Preview and In-Question Support | `US-TERM-01`, `US-TERM-02`, `US-TERM-03` | Owner: delivered [`VS-010A`](../delivery/VS-010A-chinese-terminology.md) (`VS-010A-R10-accepted`; `D-01` loop A; product-owner journeys and visual review 2026-08-22). |
...
| 5.3 | Hints, Language Assistance, and Solutions | `US-TERM-01`, `US-HINT-01`, `US-AGENT-04` | Owners: delivered `VS-009` mathematical tiered hints + post-submit solutions (`VS-009-R8-accepted`); delivered [`VS-010A`](../delivery/VS-010A-chinese-terminology.md) Chinese word/phrase Language help (`VS-010A-R10-accepted`; product-owner journeys and visual review 2026-08-22). [`VS-011`](../delivery/VS-011-contextual-text-qa.md) `CONTRACT_READY` (`VS-011-R7-accepted`) records in-progress Ask as `AGENT_QA` strong assistance (`D-01`). |
...
| 6.3 | Contextual Questions and Answers | `US-AGENT-04`, `US-AGENT-06` | Owners: [`VS-011`](../delivery/VS-011-contextual-text-qa.md) **`CONTRACT_READY`** (`VS-011-R7-accepted`; text Q&A, `US-AGENT-04`; frontend wave 1 Ask chrome on production hosts, slice revision 10; hybrid authorised search; `D-01`–`D-09` approved 2026-09-01; mock report still `VS-013`); proposed later follow-on after `VS-010B` + first `VS-011` for derived video answers (`US-AGENT-06`, not `VS-012`). |
...
| 14.4 | Syllabus and Learning-Content Management | `US-ADM-02`, `US-ADM-03`, `US-ADM-08` | Owners: delivered `VS-005`; proposed `VS-010B` for optional reviewed short-video assets (lesson/remediation attachment; question-explanation surface deferred to an additive follow-on). Localized syllabus outline items and platform objectives are mapped cleanly, and the admin maintains official source link(s) per published language edition (en and/or zh-CN). |
...
| 14.9 | AI and Content Quality Management | `US-SUPPORT-02`, `US-ADM-06` | Owners: proposed `VS-020`, `VS-021`; `VS-011` `CONTRACT_READY` persists insufficient-evidence / low-confidence flags and trajectory steps but does not ship student report or the admin queue (those two ship together); `VS-010B` / later `US-AGENT-06` keep generated video in Draft until human review. |
